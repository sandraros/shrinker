...
CLASS ltc_get_whole_abap_statement IMPLEMENTATION.
  ...
  METHOD several_lines.
    ...
    DATA(aaa) = zcl_shrinker_abap_scan=>get_whole_abap_statement(
                line_index       = 1
    ...
    cl_abap_unit_assert=>assert_equals(
        act = aaa
        exp = VALUE zcl_shrinker_abap_scan=>ty_get_next_lines_of_statement(
            first_line_index = 1
            last_line_index  = 3
            offset           = 0
            length           = 44
...
CLASS ltc_raw_scan_lines_around IMPLEMENTATION.
  ...
  METHOD c_w_lon_lf_w_ma_lf_w_dot.
    ...
    lcl_abap_statement_at_cursor=>raw_scan_lines_around(
        EXPORTING
            it_source = VALUE zcl_shrinker_abap_scan=>ty_abap_source_code(
                        ( `A:` )
                        ( `B,` )
                        ( `C.` ) )
            i_linenr  = 1
...
CLASS ltc_scan_abap_statement IMPLEMENTATION.
  ...
  METHOD c_w_lon_lf_w_ma_lf_w_dot.
    scan_result = zcl_shrinker_abap_scan=>get_abap_statement_at_cursor(
            it_source = VALUE zcl_shrinker_abap_scan=>ty_abap_source_code(
                        ( `A:` )
                        ( `B,` )
                        ( `C.` ) )
