...
CLASS zcl_shrinker_any_program DEFINITION LOCAL FRIENDS
    ltc_replace_texts.
...
CLASS ltc_replace_texts IMPLEMENTATION.
  ...
  METHOD member_interface_prefix.
    DATA(abap_source_code) = VALUE zcl_shrinker_any_program=>ty_abap_source_code(
            ( ` lo_cut ?= lo_cut->zif_abapgit_ajson~slice( '/issues' ).` ) ).
    DATA(cut) = zcl_shrinker_any_program=>create( ).
    cut->replace_texts(
        EXPORTING
    ...
    cl_abap_unit_assert=>assert_equals(
        ...
        exp = VALUE zcl_shrinker_any_program=>ty_abap_source_code(
